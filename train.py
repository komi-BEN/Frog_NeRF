...
save_points = [2, 8]  # 7分钟和15分钟保存模型
save_points_sec = sorted([minute * 60 for minute in save_points])  # 转换为秒并排序
saved_points = set()  # 跟踪已保存的点
...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            for save_point in save_points_sec:
                if save_point not in saved_points and elapsed >= save_point:
                    print("\n[time {}s] Saving Gaussians".format(elapsed))
                    scene.save(iteration)
                    saved_points.add(save_point)
...
def evaluate(model_paths, visible_count=None, wandb=None, tb_writer=None, dataset_name=None, logger=None):
    ...
    for method in os.listdir(test_dir):
        ...
        for idx in tqdm(range(len(renders)), desc="Metric evaluation progress"):
            ssims.append(fast_ssim(renders[idx].unsqueeze(0), gts[idx].unsqueeze(0)))
